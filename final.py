...
if not os.environ.get("GOOGLE_API_KEY"):
    print("WARNING: GOOGLE_API_KEY environment variable not set. The app may fail.")
...
def predict():
    ...
    if not all([user_query, domain, documents_b64]):
        return jsonify({"error": "Request must include 'query', 'domain', and a list of 'documents'"}), 400

    if domain not in PERSONALITIES:
        return jsonify({"error": f"Invalid domain. Available: {list(PERSONALITIES.keys())}"}), 400

    try:
        # Create a temporary directory to save the decoded files
        with tempfile.TemporaryDirectory() as temp_dir:
            file_paths = []
            for i, doc_b64 in enumerate(documents_b64):
                file_content = base64.b64decode(doc_b64)
                # We save it with a generic name as we don't know the original extension
            ...
            if topic != "General Inquiry":
                ...
                if not relevant_docs: relevant_docs = vector_store.similarity_search(user_query, k=8)
            else:
